...
Contributor-focused diagrams that explain how mem-rag ingests documents, runs RAG + memory, and evaluates/optimizes configs.

## Functional Flows (Mermaid)
...
### RAG Chat Pipeline (Deterministic)
...
## Physical / Runtime View (Image-Style SVG)
...
## Optimization Visual (Image-Style SVG)
...
- Diagram sources map to key files in `src/ingestion/`, `src/rag/`, `src/memory/`, `src/eval/`, and `src/optimize/`.
- For runtime behavior, see `src/cli.ts` for command wiring and `docs/EXPERIMENT.md` for the experiment flow.
